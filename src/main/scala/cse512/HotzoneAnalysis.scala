...
object HotzoneAnalysis {
  ...
  def runHotZoneAnalysis(spark: SparkSession, pointPath: String, rectanglePath: String): DataFrame = {
    ...
    val sortedDf = spark.sql("select rectangle, count(point) as pointsCount from joinResult group by rectangle order by rectangle asc")
    sortedDf.createOrReplaceTempView("sortResult")
    sortedDf.show()
